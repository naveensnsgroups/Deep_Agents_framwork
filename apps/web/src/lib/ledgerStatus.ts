...
export const LEDGER_STATUS_META: Record<string, { icon: LucideIcon; className: string; label: string }> = {
  verified: { icon: CheckCircle2, className: "text-green-400", label: "verified" },
  converted: { icon: FileCheck, className: "text-blue-300", label: "converted" },
  pending: { icon: Circle, className: "text-neutral-400", label: "pending" },
...
export function ledgerStatusMeta(status: string) {
  return LEDGER_STATUS_META[status] ?? { icon: Circle, className: "text-neutral-400", label: status };
}
