...
export function ProvenancePanel({ provenance }: { provenance: ReadProvenance[] }) {
  ...
  return (
    <div className="mt-2 border-t border-neutral-800 pt-2">
      {flagged.length > 0 ? (
        <div className="mb-2 flex items-start gap-1.5 rounded border border-amber-700/50 bg-amber-950/40 px-2 py-1.5">
          <AlertTriangle className="mt-0.5 h-3.5 w-3.5 flex-none text-amber-400" />
          <div className="min-w-0 text-[11px] leading-relaxed text-amber-200">
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex w-full cursor-pointer items-center gap-1.5 text-left text-[11px] text-neutral-500 hover:text-neutral-300"
      >
          ...
          {provenance.map((entry, i) => (
            <div key={`${entry.target}-${i}`} className="min-w-0 text-[11px]">
              <div className="flex items-baseline gap-1.5">
                ...
                <span className="break-all text-neutral-300">{entry.target}</span>
              </div>

              {entry.signals.map((signal, j) => (
                <div key={j} className="mt-1 border-l-2 border-amber-700/60 pl-2">
                  <div className="text-[10px] uppercase tracking-wide text-amber-500">{signal.label}</div>
                  {/* The matched line verbatim. Rendered as plain text, never markdown — the
                      whole point is showing exactly what the file said. */}
                  <div className="mt-0.5 break-words font-mono text-[10px] leading-relaxed text-amber-200/90">
                    {signal.excerpt}
                  </div>
                </div>
              ))}
